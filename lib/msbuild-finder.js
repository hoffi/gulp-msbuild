...
module.exports.find = function (options) {
  ...
  if (version === '12.0' || version === '14.0') {
    var program_files = is64Bit ? 'ProgramFiles(x86)' : 'ProgramFiles';
    var pathRoot = process.env[program_files] || path.join('C:', is64Bit ? 'Program Files (x86)' : 'Program Files');

    return path.join(pathRoot, 'MSBuild', version, 'Bin/MSBuild.exe');
  }
